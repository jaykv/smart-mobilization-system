...
# load data
engine = create_engine('sqlite:////var/www/html/drpy/data/DisasterResponse.db')
df = pd.read_sql_table('DisasterResponse', engine)

# load model
model = joblib.load("/var/www/html/drpy/models/classifier.pkl")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--query", help="query a message")

    args = parser.parse_args()

    if args.query:
        classification_labels = model.predict([args.query])[0]
        classification_results = dict(zip(df.columns[4:], classification_labels))
